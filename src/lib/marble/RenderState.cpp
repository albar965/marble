...
QString RenderState::Private::toString(const RenderState& state, int level) const
{
  ...
  switch(state.status())
  {
    case Marble::Complete:         status = "Complete";
      break;
    case Marble::WaitingForUpdate: status = "Waiting for update";
      break;
    case Marble::WaitingForData:   status = "Waiting for data";
      break;
    case Marble::Incomplete:       status = "Incomplete";
      break;
  }
