...
public:
  ...
  virtual QImage tile(int n, int m, int maxTileLevel)
  {
    ...
    {
      mDebug()
        << QString("TileCreator::createTiles() The size of the final image will measure  %1 x %2 pixels").arg(stdImageWidth).arg(
        stdImageHeight);
    }
          ...
          for( int i = 0; i < writtenTile.size().width(); ++i)
          {
            for( int j = 0; j < writtenTile.size().height(); ++j)
            {
              if(writtenTile.pixel(i, j) != tile.pixel(i, j))
              {
                ...
                qWarning() << "***** pixel" << i << j << "is off by" << (pixel - writtenPixel) << "pixel" << pixel << "writtenPixel"
                           << writtenPixel;
