...
void NewstuffModelPrivate::changeNode(QDomNode& node, QDomDocument& domDocument, const QString& key, const QString& value,
                                      NodeAction action)
{
  if(action == Append)
  {
...
void NewstuffModel::mapInstalled(int exitStatus)
{
  ...
  else
  {
    ...
    emit installationFailed(d->m_currentAction.first,
                            QString("Unable to unpack file. Process exited with status code %1.").arg(exitStatus));
  }
  QModelIndex const affected = index(d->m_currentAction.first);
